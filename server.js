      ...
      db.run(
        ...
        async function (err) {
          ...
          await transporter.sendMail({
            from: "Anim-Formation",
            to: email,
            subject: "Accès à Anim-Formation 🎉",
            html: `
              <h2>Paiement confirmé</h2>
              <p>Merci pour votre achat.</p>
              <p>Accédez à votre formation via ce lien sécurisé :</p>
              <a href="https://anim-formation.onrender.com/formation/${token}">Accéder à la formation</a>
...
app.get("/formation/:token", (req, res) => {
  ...
  db.get("SELECT * FROM users WHERE token=? AND paid=1", [token], (err, row) => {
    ...
    // Anti partage IP
    if (!row.ip) {
      db.run("UPDATE users SET ip=? WHERE token=?", [userIP, token]);
    } else if (row.ip !== userIP) {
      return res.send("Accès bloqué : lien utilisé sur un autre appareil.");
    }
